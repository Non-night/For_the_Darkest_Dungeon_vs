...
namespace For_the_Darkest_Dungeon.Completion
{
	...
	internal class OverrideCompletionSourceProvider : ICompletionSourceProvider
	{
		public ICompletionSource TryCreateCompletionSource(ITextBuffer textBuffer)
		{
			return new OverrideCompletionSource(textBuffer);
		}
	}

	internal class OverrideCompletionSource : ICompletionSource
	{
		private readonly ITextBuffer _buffer;
		private bool _isDisposed = false;
		private readonly Regex _headerRegex = new Regex(@"^[a-zA-Z0-9_]+:", RegexOptions.Compiled);

		public OverrideCompletionSource(ITextBuffer buffer)
		{
			_buffer = buffer;
		}
		...
		public void AugmentCompletionSession(ICompletionSession session, IList<CompletionSet> completionSets)
		{
			...
			if (resultList != null && resultList.Count > 0)
			{
				var completions = resultList.Select(k =>
					new Microsoft.VisualStudio.Language.Intellisense.Completion(k, k, "Darkest Override Context: " + activeHeader, null, null)).ToList();
